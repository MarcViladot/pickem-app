...
export default {
    ...
    getAll() {
        return API.get(`round/translationGroups`)
            .then((r: AxiosResponse<ResponseApi<TranslationGroup[]>>) => {
                return r.data
            }, (err: AxiosError) => {
    ...
    newTranslationGroup(data: TranslationGroupDto) {
        return API.post(`round/translationGroup`, data)
            .then((r: AxiosResponse<ResponseApi<TranslationGroup>>) => {
                return r.data
            }, (err: AxiosError) => {
                return new ResponseServerError(err);
